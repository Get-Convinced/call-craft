...
.brand{font-weight:800;letter-spacing:-.3px}.nav{display:flex;gap:6px;margin-left:6px}
.nav a{font-size:13.5px;border:1px solid var(--line);padding:7px 13px;border-radius:999px;color:#444}
...
const GLABEL={discovery:'Discovery',qualification:'Qualification',solutioning:'Solutioning',deal_control:'Deal control',urgency:'Urgency'};
...
const ARCH={"value-not-quantified":"Value never quantified on the buyer's numbers","buyer-gated-deferral":"Buyer-gated deferral accepted, no forcing function","no-pain-feature-tour":"No-pain feature tour against an incumbent","economic-buyer-never-met":"Sold to a user, never met the economic buyer","deferred-proof-loop":"Proof deferred to a session that never came","structurally-dead-disqualify":"Structurally dead, parked instead of disqualified","lost-to-competitor":"Lost to an entrenched competitor","won":"Won","still-active":"Still active"};
...
const MCRM={
  MFL:{metric:'Metric',economic_buyer:'Economic buyer',decision_process:'Decision process',decision_criteria:'Decision criteria',champion:'Champion',pain:'Pain',competition:'Competition',paper:'Paper process'},
...
function viewCouncil(){
  ...
  (c.seats||[]).forEach(s=>{
    ...
    [['Why deals stall',s.answer_why_deals_stall],['What reps miss',s.answer_what_reps_miss],['Who solutions',s.answer_who_solutions],['The one change',s.answer_one_change]].filter(a=>a[1]).forEach(a=>h+=`<div class="small" style="margin-top:5px"><b>${a[0]}:</b> ${esc(a[1])}</div>`);
...
function route(){
  ...
  document.querySelectorAll('.nav a').forEach(a=>a.classList.toggle('on',a.getAttribute('href')==='#'+(['org','reps','deals','crm','council'].includes(v)?v:'org')));
  window.scrollTo(0,0);
}
...
<title>{html.escape(org)} call audit</title>
<style>{CSS}</style></head><body>
<div class="top"><div class="shell">
  <span class="brand">{html.escape(org)} call audit</span>
  <div class="nav"><a href="#org">Diagnosis</a><a href="#reps">Reps</a><a href="#deals">Deals</a><a href="#crm">CRM check</a><a href="#council">Council</a></div>
  <div class="searchwrap"><input id="q" placeholder="Search reps, deals, calls..." autocomplete=off><div id="results"></div></div>
</div></div>
<div id="app"></div>
<footer><div class="shell">{html.escape(org)} per-call audit · craft scored apart from outcome · {html.escape(rdate)}. Numbers aggregate; open any call to see how the read was made.</div></footer>
