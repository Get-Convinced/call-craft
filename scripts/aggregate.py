...
def main():
    ...
    deals={d["deal_id"]:d for d in A.read_json(os.path.join(wd,"canonical","deals.json"))}
    notes=A.read_json(os.path.join(wd,"canonical","notes.json"))
    ...
    for co in callout:
        ...
        for r in co.get("reps",[]):
            ...
            reps_pay.append({"name":r["rep_name"],"archetype":arch,"composite":cmp,
    ...
    def owned_outcomes():
        ...
        for d in deals.values():
            if A.excluded_rep(d.get("owner",""),cfg): continue
            owned[A.norm_name(A.canonical_rep_name(d.get("owner",""),cfg))].append(d)
        return owned
    ...
    for key, cl in rep_calls.items():
        ...
        arch=(role_by_key.get(key,{}) or {}).get("archetype") or cl[0].get("archetype") or "ae-closer"
        ...
        reps_out.append({"key":key,"slug":A.slug(role_by_key.get(key,{}).get("name") or cl[0]["account"] if False else key),
            ...
            "n_calls":len(cl),"rank_eligible":len(comps)>=cfg.get("min_calls_to_rank_rep",3),
            "dim_avgs":dim_avgs,
    ...
    for ui,u in opp.items():
        ...
        deals_out.append({"unit_index":ui,"slug":A.slug(acct or f"deal-{ui}"),"account":acct,
            ...
            "postmortem":deslop(pm.get(ui)),"n_calls":len(cs),
