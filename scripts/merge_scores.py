...
ADHERENCE_RULES = {
    "R1": "rep-quote grounding: an evidence quote must be the rep's actual words from the transcript",
    "R2": "read the whole turn: do not floor a dimension to 1 when the rep gave a substantive reply",
    "R3": "scale anchoring: no defaulting to 3; a 4 or 5 must land, a 1 or 2 must reflect a real miss",
    "R4": "craft versus outcome firewall: score the move, not the deal result",
    "R5": "qualification earned on the call: the qualification dimensions are credited only from what the rep drew out on the call",
    "R6": "anti-slop: no buzzwords or stray dashes in authored prose",
}
...
def main():
    ...
    if os.path.isdir(nout):
        FIELDS = ("company", "company_aliases", "bucket", "call_status", "transcript_quality",
                  "call_phase", "is_sales_relevant", "primary_external_participants", "org_participants", "one_line")
