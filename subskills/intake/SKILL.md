...
|---|---|---|
| `transcript` | a single call's recording: speaker turns, a meeting header, attendees | Read.ai markdown/json, Gong, Fireflies, Otter, Zoom VTT, plain text, generic JSON array of turns |
| `crm_deals` | one row per opportunity/deal: account, owner, stage, amount, dates, maybe MEDDIC columns | CSV, XLSX, JSON |
...
After classifying, look at what optional buckets are missing (`crm_deals`, `crm_notes`,
`stage_history`, `company_doc`). Ask the operator **one** consolidated question: do they have any of
these that were not in the folder? Make clear each is optional and what each one buys:

- **CRM deal export** turns the craft scores into a deal-by-deal audit, a funnel, and a CRM-vs-call
  trust check. Without it the audit is craft-only (still useful).
- **Rep notes** add follow-through evidence between calls.
- **Stage history** lets the funnel show real progression instead of a current-stage snapshot.
- **Company docs** sharpen the product/competitor context the scorer grounds on.
