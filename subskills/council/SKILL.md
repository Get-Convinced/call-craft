...
1. **Pipeline diagnostician.** Where do deals actually die, and is it a craft problem or a structural
   one? Reads the funnel and the stall pile.
2. **Sales-craft coach.** What do reps systematically do well and badly across the rubric? Which
   dimensions are the org's floor? Separate role-appropriate gaps from real weaknesses.
3. **Deal-strategy partner.** Who is doing the actual solutioning and progression? Where is the org
   single-threaded or founder-dependent?
4. **Enablement skeptic.** What would coaching individuals NOT fix: process, qualification discipline,
   knowledge gaps, missing deal hygiene? Be adversarial.
