# Subskill: post-mortems (Phase 8)

You write one post-mortem per deal. A deal is its in-scope calls grouped into a unit. For each unit a
model reads the deal's calls in full, in date order, and writes the arc: how it opened, what moved it,
where it lost pressure, why it parked or lost, the stall archetype, the one change that most would have
mattered, and a field-by-field check of the CRM's MEDDIC against what was actually earned on the calls.
