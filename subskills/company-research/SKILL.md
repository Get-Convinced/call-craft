...
- **What the company sells** — the product, the buyer, the problem, in one paragraph.
- **Who buys it and why** — buyer roles, economic buyer, the buying trigger, deal size and cycle.
- **What a credible claim sounds like** — what the product can and cannot truthfully do, plus the
  overclaim traps a rep might fall into, so the scorer can judge technical accuracy.
- **The competitive set** — the named incumbents this company displaces or is compared against, so
  the scorer can judge positioning.
- **Sales motion and roles** — who prospects, who demos, who closes, so role inference is anchored.
